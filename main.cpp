...
//this multidimensional array stores the distance matrix , fully converged  for easier coding
 ...
 int fitnessfn(int array[V])// calculates route fitness 
 {

	 int fitness = 0;
	 for (int i = 0; i < V - 1; i++)
	 {
		 // cout << matrix[array[i]][array[i + 1]]<<endl;
		 fitness += (matrix[array[i]][array[i + 1]]);
 ...
 void reproduce(route &childA, route &childB, route parentA, route parentB) // function will identify parents by ID 
 {
	 ...
	 */
	 ...
	 int i = 1; // counter var
	 while (i != (V+1)) // while the last value of second child DNE the first value in the first parent
	 {  
		 ...
		 childB.path[i-1] = secondCycle; // after two calls, we have a value we can add to the second Child
		
		 int forChA = crossoverhelper(parA, parB, secondCycle); // call crossover one more time   
		 childA.path[i] = forChA; // now we have a value for first child. 
		 a = forChA;
		 i++;
	 }
 ...
 void currentPop(vector<route>& pool, int g) //uses reproduction to create a new population
 {
 ...
 void poolsort(int n, vector <route> &pool ) // sorts by fitness using comparebyfitness fn
 {
 ...
 int main()
	{
		...
		poolsort(20, InitRoutePool);
		currentPop(InitRoutePool,6);
